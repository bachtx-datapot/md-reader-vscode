...
| Patterns | Conformance to `.claude/patterns/*` — check each convention |
...
### 5. Pattern Conformance Scoring

For each file changed, score against relevant pattern from `.claude/patterns/`:

| Convention | Weight | Check |
|------------|--------|-------|
| baseColumns used (schema) | 2 | Grep for `...baseColumns` |
| Money = bigint (schema) | 2 | No `numeric()` or `decimal()` for VND |
| permissionProcedure (router) | 3 | All mutations use permissionProcedure |
| createModuleLogger (service) | 1 | No console.log in production |
| emitOutboxEvent (events) | 2 | No publishEvent() |
| data-testid (UI) | 1 | All interactive elements have testid |
| Recipe components (UI) | 2 | Uses DataTable/CrudSheet/ListPage/DetailPage |
| 5-scenario tests | 2 | Happy + validation + not-found + auth + business rule |

**Score: {sum}/{max} — report as percentage.**

### Lazy Fix Penalty

When reviewing code that modifies existing files (not new code):
- Check if developer loaded full module context before modifying (see `.claude/rules/full-context-loading.md`)
- If change is narrow grep→edit→done pattern on Tier 1/2 changes: **-2 points from overall score**
- Note in review: "Lazy fix detected — modification without full context loading"
