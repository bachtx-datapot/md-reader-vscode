...
3. **Pattern + Spec Loading (MANDATORY — Before ANY Implementation)**
   Follow `.claude/rules/full-context-loading.md` before implementation.
   Follow `.claude/rules/trigger-table.md` to determine which patterns and specs to load.

   | File Type | Must Read |
   |-----------|-----------|
   | Schema (`schema/*.ts`) | `.claude/patterns/schema.md` + `04-database-design-{domain}.md` |
   | Router (`router*.ts`) | `.claude/patterns/router.md` + `05-api-design-{domain}.md` |
   | Service (`services/*.ts`) | `.claude/patterns/service.md` + `.claude/patterns/event.md` |
   | UI Page (`app/**/*.tsx`) | `.claude/patterns/ui-page.md` + `.claude/patterns/ui-table.md` |
   | UI Component (`components/**`) | `.claude/patterns/ui-form.md` |
   | Tests (`tests/**`) | `.claude/patterns/test.md` |
...
- Specs loaded: [list]
- Conformance check: [pass/fail per convention]
