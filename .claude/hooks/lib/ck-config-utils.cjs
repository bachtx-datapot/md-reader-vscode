...
function getReportsPath(planPath, resolvedBy, planConfig, pathsConfig, baseDir = null) {
  ...
  // Issue #335 fix: When planPath is absolute (session-resolved), reportPath is already
  // absolute — skip joining with baseDir to prevent path duplication
  // (path.join doesn't reset on absolute second arg like path.resolve does)
  if (baseDir && !path.isAbsolute(reportPath)) {
